...
func SetupRouterAndRoutes(config *config.Config, deps *config.Dependencies) chi.Router {
	...
	router.Route("/api/v1/follow/user/{user_id}", func(r chi.Router) {
		r.Use(middleware.SetHeader("Content-Type", "application/json"))
		r.Use(ddchi.Middleware(ddchi.WithServiceName(config.ServiceName)))

		r.Post("/follow", createFollow(deps))
		r.Get("/followings", getFollowings(deps))
		r.Get("/followers", getFollowers(deps))
